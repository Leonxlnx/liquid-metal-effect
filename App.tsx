...
export default function App() {
  const [appMode, setAppMode] = useState<'timer' | 'alarm'>('timer'); 
  const [status, setStatus] = useState<'idle' | 'running' | 'paused' | 'ringing'>('idle');
  
  // Timer State (Countdown)
  const [timerInput, setTimerInput] = useState({ h: 0, m: 8, s: 0 }); // Default 8 min
  const [remainingSeconds, setRemainingSeconds] = useState(0);
  const endTimeRef = useRef<number>(0);
  
  // Alarm State (Clock Target)
  const [alarmInput, setAlarmInput] = useState({ h: 8, m: 30 });
  ...
  // Massively increased width to give room for liquid effect
  const digitWidth = isMobile ? 100 : 350; 
  const digitHeight = isMobile ? 140 : 350;
  const separatorWidth = isMobile ? 30 : 100;
    ...
    const tick = () => {
      ...
      if (status === 'running') {
        if (appMode === 'timer') {
          ...
          // Alarm: Check if Current Time matches Target
          ...
          if (
            ...
            // Trigger once within the second
  ...
  const handleStart = () => {
    if (appMode === 'timer') {
      ...
      // Alarm Mode: Just activate checks
      setStatus('running');
    }
  };
  ...
  if (appMode === 'timer') {
    // Timer Mode: Always show inputs or countdown
    if (status === 'idle') {
    ...
    if (status === 'idle') {
       ...
       // Alarm Active: Show CURRENT CLOCK TIME
  ...
  // Show seconds if Timer (always) or if Alarm is Running (Clock Mode)
  const showSeconds = appMode === 'timer' || (appMode === 'alarm' && status === 'running');
      ...
      <div className="relative w-full h-screen bg-[#050505] text-[#e0e0e0] flex flex-col justify-center items-center overflow-hidden font-['Inter'] selection:bg-white selection:text-black">
        ...
        <main className="relative z-10 flex flex-col items-center justify-center w-full">
          ...
          {/* 
            ...
            Increased gap to 8 (md:gap-16) to provide more "left/right space".
          */}
          <div className="flex items-center justify-center gap-2 md:gap-4">
             ...
             {status === 'idle' ? (
                 <div className="relative group z-30">
                    <input 
                       ...
                       className="w-[25vw] md:w-[320px] bg-transparent text-center text-[20vw] md:text-[14rem] leading-none font-bold text-transparent bg-clip-text bg-gradient-to-b from-white via-neutral-200 to-neutral-500 outline-none caret-white/50"
                    />
             ...
             {/* SEPARATOR (Always visible if seconds are visible, or just H:M for alarm input) */}
          ...
          {appMode === 'alarm' && status === 'running' && (
             <div className="mt-12 flex flex-col items-center gap-2">
                 <div className="text-xs tracking-[0.4em] uppercase text-white/40">Current Time</div>
                 <div className="text-sm tracking-[0.2em] uppercase text-white/80 animate-pulse">
            ...
            <AnimatePresence mode="wait">
              ...
              {status === 'ringing' && (
                ...
                >
                   ...
                   <div className="text-4xl md:text-6xl uppercase tracking-[0.2em] font-black text-white animate-[pulse_0.1s_infinite]">
                     {appMode === 'timer' ? "00:00:00" : "WAKE UP"}
                   </div>
